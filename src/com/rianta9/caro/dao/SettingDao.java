...
public class SettingDao {
	public static Setting LoadSettingInfo() {
		...
		try {
			...
			// Đọc difficulty (có thể không có trong file cũ)
			int difficulty = Value.DEFAULT_DIFFICULTY;
			String difficultyLine = reader.readLine();
			if (difficultyLine != null && !difficultyLine.trim().isEmpty()) {
				try {
					difficulty = Integer.parseInt(difficultyLine.trim());
				} catch (NumberFormatException e) {
					difficulty = Value.DEFAULT_DIFFICULTY;
				}
			}
			
			Setting setting = new Setting(backgroundColor, cellColor, xColor, oColor, mode, difficulty);
			reader.close();
			return setting;
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static void SaveSettingInfo(Setting setting) {
		...
		try {
			...
			writer.write(String.valueOf(setting.getDifficulty()) + "\n");
